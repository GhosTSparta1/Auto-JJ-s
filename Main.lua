...
local require = function(Name) return loadstring(game:HttpGet(('https://raw.githubusercontent.com/GhosTSparta1/Auto-JJ-s/main/%s.lua'):format(Name)))() end
...
local function ListenChange(Obj) 
    ...
    Obj.FocusLost:Connect(function() 
        ...
        if not CurrentText or string.match(CurrentText, "^%s*$") then 
            return 
        end 
...
local function DoJJ(n, prefix, jump)
    ...
    if success then
        ...
        -- Agachar e levantar (pressionando a tecla 'C' duas vezes)
        Char:PressKey("C")
        task.wait(0.2)  -- Aguarda um pouco antes de levantar
        Char:PressKey("C")
        task.wait(0.2)  -- Aguarda antes de pular

        -- Pular
        if jump then
            Char:Jump()
            task.wait(0.2) -- Aguarda o salto
        end
        
        -- Realiza um 360 com aleatoriedade de -2 a +2 graus
        local rotation = 360 + math.random(-2, 2)
        Char:SetRotation(rotation)

        task.wait(Options.Tempo) -- Espera o tempo configurado
    end
end
